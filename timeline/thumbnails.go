...
func (task thumbnailTask) generateThumbnail(ctx context.Context, inputFilename string, inputBuf []byte) ([]byte, string, error) {
	...
	case strings.HasPrefix(task.DataType, "image/") || task.DataType == "application/pdf":
		inputImage, err := tlzmedia.LoadImageVips(task.logger, inputFilename, inputBuf)
		if err != nil {
...
func (tl *Timeline) GetItemContent(ctx context.Context, ir ItemRow) (filePath string, data []byte, err error) {
	if ir.DataFile != nil {
		return filepath.Join(tl.repoDir, filepath.FromSlash(*ir.DataFile)), nil, nil
	}
	if ir.DataText != nil {
		return "", []byte(*ir.DataText), nil
	}
	if ir.DataID != nil {
		err = tl.db.ReadPool.QueryRowContext(ctx,
			`SELECT content FROM item_data WHERE id=? LIMIT 1`, *ir.DataID).Scan(&data)
	}
	return
}
...
func (tl *Timeline) GeneratePreviewImage(ctx context.Context, itemRow ItemRow, ext string, obfuscate bool) ([]byte, error) {
	...
	imageBytes, err := tlzmedia.LoadAndEncodeImage(Log.Named("previewer"), inputFilePath, inputBuf, ext, maxPreviewImageDimension, obfuscate)
	if err != nil {
